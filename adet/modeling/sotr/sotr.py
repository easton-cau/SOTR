...
class SOTR(nn.Module):
    def __init__(self, cfg):
        ...
        self.strides = cfg.MODEL.SOTR.FPN_INSTANCE_STRIDES
        self.sigma = cfg.MODEL.SOTR.SIGMA
        self.num_classes = cfg.MODEL.SOTR.NUM_CLASSES
        self.num_kernels = cfg.MODEL.SOTR.NUM_KERNELS
        self.num_grids = cfg.MODEL.SOTR.NUM_GRIDS

        self.instance_in_features = cfg.MODEL.SOTR.INSTANCE_IN_FEATURES
        self.instance_strides = cfg.MODEL.SOTR.FPN_INSTANCE_STRIDES
        self.instance_in_channels = cfg.MODEL.SOTR.INSTANCE_IN_CHANNELS
        self.instance_channels = cfg.MODEL.SOTR.INSTANCE_CHANNELS

        self.mask_on = cfg.MODEL.MASK_ON
        self.mask_in_features = cfg.MODEL.SOTR.MASK_IN_FEATURES
        self.mask_in_channels = cfg.MODEL.SOTR.MASK_IN_CHANNELS
        self.mask_channels = cfg.MODEL.SOTR.MASK_CHANNELS
        self.num_masks = cfg.MODEL.SOTR.NUM_MASKS

        self.resize_input_factor = cfg.MODEL.SOTR.RESIZE_INPUT_FACTOR
        # self.confidence_score = cfg.MODEL.SOTR.CONFIDENCE_SCORE
        self.max_before_nms = cfg.MODEL.SOTR.NMS_PRE
        self.score_threshold = cfg.MODEL.SOTR.SCORE_THR
        self.update_threshold = cfg.MODEL.SOTR.UPDATE_THR
        self.mask_threshold = cfg.MODEL.SOTR.MASK_THR
        self.max_per_img = cfg.MODEL.SOTR.MAX_PER_IMG
        self.nms_kernel = cfg.MODEL.SOTR.NMS_KERNEL
        self.nms_sigma = cfg.MODEL.SOTR.NMS_SIGMA
        self.nms_type = cfg.MODEL.SOTR.NMS_TYPE
        ...
        self.cate_head = TwinTransformerHead(cfg, instance_shapes)

        mask_shapes = [backbone_shape[f] for f in self.mask_in_features]
        self.mask_head = MaskHead(cfg, mask_shapes)


        self.ins_loss_weight = cfg.MODEL.SOTR.LOSS.DICE_WEIGHT
        self.focal_loss_alpha = cfg.MODEL.SOTR.LOSS.FOCAL_ALPHA
        self.focal_loss_gamma = cfg.MODEL.SOTR.LOSS.FOCAL_GAMMA
        self.focal_loss_weight = cfg.MODEL.SOTR.LOSS.FOCAL_WEIGHT
    ...
    def forward(self, batched_inputs):
        ...
        mask_features[3] = mask_extra_feat[0]
    ...
    def preprocess_image(self, batched_inputs):
        ...
        #     images = [self.Reize_the_input(x) for x in images]
    ...
    def inference_single_image(
        ...
        # filter the instance with score thresh
        # if not self.training:
...
class TwinTransformerHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.instance_in_channels = cfg.MODEL.SOTR.INSTANCE_IN_CHANNELS
        self.instance_channels = cfg.MODEL.SOTR.INSTANCE_CHANNELS
        self.type_dcn = cfg.MODEL.SOTR.TYPE_DCN
        ...
        assert in_channels == cfg.MODEL.SOTR.INSTANCE_IN_CHANNELS, \
            print("In channels should equal to tower in channels!")

        self.transformer = TwinTransformer(dim=256)
        ...
        prior_prob = cfg.MODEL.SOTR.PRIOR_PROB
    ...
    def forward(self, features):
        ...
        for idx, feature in enumerate(features):
            ...
            feat = F.interpolate(feature, size=seg_num_grid, mode='bilinear')

            transformer_feat = self.transformer(feat).permute(0, 2, 3, 1)

            cate_pred_single = self.cate_pred(transformer_feat).permute(0, 3, 1, 2)
            cate_pred.append(cate_pred_single)

            kernel_pred_single = self.kernel_pred(transformer_feat).permute(0, 3, 1, 2)
            kernel_pred.append(kernel_pred_single)

            if idx==3:
                mask_transformer_feat = self.transformer(feature)
                mask_pred.append(mask_transformer_feat)

        return cate_pred, kernel_pred, mask_pred
...
class MaskHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        norm = None if cfg.MODEL.SOTR.NORM == "none" else cfg.MODEL.SOTR.NORM
